...
class e4WorkflowApp
{
    ...
    public function __construct($root = false, $path = 'appConfig.json')
    {
        ...
        if (false === ($e4Config = file_get_contents($root.$path))) {
            throw new Exception('Config file "'.$path.'" not found', 1);
        }
        if (false === ($e4Config = json_decode($e4Config, true))) {
        ...
        register_shutdown_function([$this, 'exportConfig']);
        register_shutdown_function([$this, 'clearCacheFiles']);
    }
    ...
    public function setName($name, $id)
    {
        ...
        $this->cachePath = $_SERVER['HOME'].'/Library/Caches/com.runningwithcrayons.Alfred-3/Workflow Data/'.$this->id.'/';
        $this->configPath = $_SERVER['HOME'].'/Library/Application Support/Alfred 3/Workflow Data/'.$this->id.'/';
    ...
    public function addCommand($key, $configs)
    {
        ...
        if (isset($configs['default']) && true === $configs['default']) {
    ...
    public function run($argv)
    {
        ...
        if ('default' != $argv[1] && count($this->appCommands) > 0) {
        ...
        if (!count($objects) && false !== $this->appDefaultCommand) {
            $out = $this->loadCommander($this->appDefaultCommand, $query)->run($query, $argv);
        } elseif (1 == count($objects) && false !== ($data = $objects[0]->getQueryMatch())) {
    ...
    public function sendHTTPRequest($url, $post = null, $ttl = 300)
    {
        ...
        if (200 != curl_getinfo($ch, CURLINFO_HTTP_CODE) || !$response) {
            $response = false;
        } else {
